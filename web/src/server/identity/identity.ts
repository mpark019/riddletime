...
export async function getCurrentProfile(): Promise<Profile | null> {
  ...
  const { rows } = await pool.query(
    "select id, name, display_name, avatar_url, role from profiles where id = $1",
    [user.id],
  );
  const row = rows[0];
  return row
    ? { id: row.id, name: row.name, displayName: row.display_name, avatarUrl: row.avatar_url, email: user.email ?? null, role: row.role }
    : null;
}
...
export async function requireProfile(client: PoolClient): Promise<Profile> {
  ...
  const { rows } = await client.query(
    "select id, name, display_name, avatar_url, role from profiles where id = $1 for update",
    [user.id],
  );
  const row = rows[0];
  if (!row) {
  ...
  return { id: row.id, name: row.name, displayName: row.display_name, avatarUrl: row.avatar_url, role: row.role };
}
